...
def amap_line(config):
    try:
        key = config["key"]
        line_out_fp = config["line_out_fp"]
    ...
    if out_dir is not None:
        line_out_fp = os.path.join(out_dir, line_out_fp)
    ...
    try:
        ...
        for i in range(0, len(poi_gdf)):
            ...
            line = poiinfo.amap_poiinfo_line(key, id)
            ...
            if line is None:
                no_line_cnt += 1
                print(f"{no_line_cnt}个POI没有边界信息")
                continue
            ...
            if i%2==0:
                ...
                gdf.to_file(line_out_fp, driver='GeoJSON', encoding="utf-8")
...
if __name__ == '__main__':
    config = {
        ...
        "poi_out_fp": "厦门市道路POI_gcj02.json",  # json格式
        "line_out_fp" : "厦门市道路边界_gcj02.json", # json格式
    }
    amap_line(config) # 爬取线状的边界信息
    # tools.PntGeoJSONToShp_WGS84(os.path.join(config["out_dir"], config["poi_out_fp"]) )  # 转成shp文件，并把坐标系从火星坐标转到WGS84
    tools.LineGeoJSONToShp_WGS84(os.path.join(config["out_dir"], config["line_out_fp"]) )# 转成shp文件，并把坐标系从火星坐标转到WGS84
